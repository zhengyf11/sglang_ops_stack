...
def test_retry_environment_check_endpoint_resets_failed_job(
    ...
    job_service.mark_failed(
        db_session,
        job,
        error_code=EnvironmentErrorCode.DOCKER_NOT_INSTALLED,
...
def test_job_page_renders_environment_steps_command_outputs_and_confirmation_form(
    client: TestClient,
    db_session: Session,
) -> None:
    host = host_service.create_host(db_session, HostCreate(name="gpu", ip="10.0.0.1"))
            ...
            "steps": [
                {
                    ...
                    "commands": [
                        {
                            "id": "ssh.smoke",
                            "summary": "Verify SSH command execution",
                            "risk": "read_only",
                            "exit_code": 0,
                            "timed_out": False,
                            "stdout": "0",
            ...
            "install_plan": [
                {
                    ...
                    "command": "sudo -n apt-get install -y docker.io",
                }
            ],
        },
    )
    ...
    assert "DOCKER_NOT_INSTALLED" in page.text
    assert "ssh.smoke" in page.text
    assert "stdout" in page.text
    assert "Confirm installation" in page.text
    assert "package_install" in page.text


def test_job_page_renders_reboot_required_continue_hint(
    client: TestClient,
    db_session: Session,
) -> None:
    host = host_service.create_host(db_session, HostCreate(name="gpu", ip="10.0.0.1"))
    ...
    job_service.mark_waiting_confirmation(
        ...
        result={
            "steps": [],
            "install_plan": [{"id": "driver.manual_install", "manual": True}],
            "reboot_required": True,
            "resume_step": "driver",
            "risk_notice": "NVIDIA driver requires reboot",
        },
    )
    ...
    assert page.status_code == 200
    assert "Reboot required" in page.text
    assert "Continue after reboot" in page.text
    assert "NVIDIA driver requires reboot" in page.text
